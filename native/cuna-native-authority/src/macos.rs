...
mod tests {
    ...
    #[test]
    fn owned_exchange_remains_closed_before_secret_copy_or_spawn() {
        assert!(!owned_exchange_runtime_admitted());
    }

    #[test]
    fn darwin_kernel_identity_layouts_are_exact() {
        assert_eq!(size_of::<AuditToken>(), 32);
        assert_eq!(size_of::<ProcessUniqueIdentifierInfo>(), 56);
        let token = audit_token_for(42_424, 17);
        assert!(token.is_ok());
        if let Ok(token) = token {
            assert_eq!(token.values, [0, 0, 0, 0, 0, 42_424, 0, 17]);
        }
    }

    #[test]
    fn code_identity_equality_is_length_and_content_sensitive() {
        assert!(constant_time_equal(
            b"code-directory-one",
            b"code-directory-one"
        ));
        assert!(!constant_time_equal(
            b"code-directory-one",
            b"code-directory-two"
        ));
        assert!(!constant_time_equal(b"short", b"longer"));
    }

    #[test]
    fn suspended_spawn_binds_real_audit_identity_and_rejects_pid_version_drift() {
        let result = exercise_suspended_system_child();
        assert!(
            result.is_ok(),
            "direct macOS child identity exercise failed: {result:?}"
        );
    }

    fn exercise_suspended_system_child() -> Result<()> {
        let executable = c_string("/usr/bin/true", "native test executable")?;
        let directory = c_string("/usr/bin", "native test working directory")?;
        let mut actions = OwnedSpawnActions::new()?;
        actions.add_chdir(&directory)?;
        let mut attributes = OwnedSpawnAttributes::new()?;
        attributes.configure_closed_child()?;
        let mut pid = 0;
        let mut argv = [executable.as_ptr().cast_mut(), null_mut()];
        let mut environment: [*mut c_char; 1] = [null_mut()];
        // SAFETY: pointers remain live and the child is returned suspended.
        let status = unsafe {
            libc::posix_spawn(
                &raw mut pid,
                executable.as_ptr(),
        ...
        if status != 0 || pid <= 0 {
            return Err(unavailable("The native test child could not be spawned."));
        }
        let mut child = OwnedChild::new(pid);
        child.require_stopped()?;
        child.require_owned_process_group()?;
        let parent = process_unique_identity(unsafe { libc::getpid() })?;
        let identity = process_unique_identity(pid)?;
        if identity.parent_unique_id != parent.unique_id {
            return Err(unavailable(
                "The native test child parent identity differed.",
            ));
        }
        let token = audit_token_for(pid, identity.id_version)?;
        let path = process_path_for_audit_token(&token)?;
        if canonical_path(&path)? != canonical_path("/usr/bin/true")? {
            return Err(unavailable("The native test child audit path differed."));
        }
        let dynamic = dynamic_code_for_process(pid, &token)?;
        validate_dynamic_code(&dynamic)?;

        let mut drifted = token;
        drifted.values[7] = drifted.values[7].wrapping_sub(1);
        if process_path_for_audit_token(&drifted).is_ok()
            || dynamic_code_for_process(pid, &drifted)
                .and_then(|code| validate_dynamic_code(&code))
                .is_ok()
        {
            return Err(unavailable(
                "A drifted native PID version survived the audit-token negative control.",
            ));
        }
        ...
        loop {
            ...
            if Instant::now() >= deadline {
                return Err(unavailable("The native test child did not exit."));
            }
            std::thread::sleep(Duration::from_millis(5));
        }
        child.terminate_descendants_and_verify()
