...
function authorityResponses({ replay = false, rulesetBypass = false } = {}) {
  ...
  return async (url, options) => {
    ...
    if (url.endsWith("/rulesets") && options.method === "GET") {
      return response(200, [{ id: 1, name: configuredAuthority.rulesetName, target: "tag", enforcement: "active" }]);
    }
    ...
    if (url.endsWith("/git/tags") && options.method === "POST") return response(201, { sha: tagSha });
...
test("checked-in authority remains fail-closed until the external tag ruleset is configured", async () => {
