...
test("the candidate-bound installed CLI completes signup/login/API-key/logout against a local contract server", INSTALLED_E2E_TEST_OPTIONS, async (t) => {
  ...
  try {
    ...
    const env = {
      ...
      // Keep the isolated credential root the same on hosted POSIX and Windows
      // runners; the product selects the normal platform directory otherwise.
      XDG_CONFIG_HOME: path.join(user, "appdata"),
      USERPROFILE: user,
      HOME: user,
    ...
    const checkedRemoteBrowserLogin = checkedFeatures.find((feature) => feature.feature === "browser_login_remote");
    ...
    }, `installed browser-login probe mismatch: ${JSON.stringify(checkedRemoteBrowserLogin)}`);
...
async function runNpm(args, options) {
  // On POSIX, npm is directly executable through PATH. On Windows, spawn
  // cannot safely execute a .cmd file with shell disabled; locate npm.cmd and
  // invoke the Node-owned npm-cli.js instead. This keeps argv shell-free while
  // exercising the same installed package transaction on every platform.
  if (process.platform !== "win32") return run("npm", args, options);
  const located = await run("where.exe", ["npm.cmd"], options);
  if (located.code !== 0) return located;
  const npmCommand = located.stdout.split(/\r?\n/u).map((line) => line.trim()).find(Boolean);
  if (npmCommand === undefined) throw new Error("npm.cmd could not be resolved from PATH");
