...
const sourceBinary = path.resolve(root, args.get("binary") ?? "native/target/release/runa-native-bridge.exe");
...
const nativePackage = metadata.packages.find((entry) => entry.name === "runa-native-bridge");
...
invariant(identity.version?.companyName === "Runa Laboratories", "Native CompanyName is missing");
invariant(identity.version?.fileDescription === "Runa Native Credential and Browser Bridge", "Native FileDescription is missing");
invariant(identity.version?.originalFilename === "runa-native-bridge.exe", "Native OriginalFilename is missing");

await mkdir(output, { recursive: true });
const binaryFile = "runa-native-bridge.exe";
const binary = path.join(output, binaryFile);
await copyFile(sourceBinary, binary);
const identityReceiptFile = "runa-native-bridge.identity.json";
...
const sbom = {
  ...
  name: `runa-native-bridge-${nativePackage.version}`,
  documentNamespace: `https://getcuna.com/spdx/native/${sourceCommit}/${sourceBinarySha256}`,
  creationInfo: {
    created: new Date().toISOString(),
    creators: ["Organization: Runa Laboratories", "Tool: runa-native-evidence-v1"],
  },
  packages,
...
const sbomFile = "runa-native-bridge.spdx.json";
...
const provenance = {
  ...
  limitations: [
    ...
    "NO_WINDOWS_OWNED_PROCESS_HANDLE_CHILD_IDENTITY_AUTHORITY",
  ],
  generatedAt: new Date().toISOString(),
};
const provenanceFile = "runa-native-bridge.provenance.json";
...
await writeExclusiveJson(path.join(output, "runa-native-bridge.manifest.json"), manifest);
