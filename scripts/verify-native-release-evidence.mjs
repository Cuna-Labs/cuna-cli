...
const manifest = await readJson(path.join(evidence, "runa-native-bridge.manifest.json"));
const provenance = await readJson(path.join(evidence, "runa-native-bridge.provenance.json"));
const sbom = await readJson(path.join(evidence, "runa-native-bridge.spdx.json"));
const identity = await readJson(path.join(evidence, "runa-native-bridge.identity.json"));
const binary = path.join(evidence, "runa-native-bridge.exe");
const binarySha256 = await sha256File(binary);
const sbomSha256 = await sha256File(path.join(evidence, "runa-native-bridge.spdx.json"));
const provenanceSha256 = await sha256File(path.join(evidence, "runa-native-bridge.provenance.json"));
const identitySha256 = await sha256File(path.join(evidence, "runa-native-bridge.identity.json"));

invariant(manifest.schema === "runa.native-bridge-manifest.v1", "Native manifest schema is invalid");
invariant(manifest.protocol === "runa.native-bridge.v1", "Native protocol identity is invalid");
...
invariant(manifest.executableFile === "runa-native-bridge.exe", "Native executable name is invalid");
...
invariant(manifest.sbom?.file === "runa-native-bridge.spdx.json" && manifest.sbom.sha256 === sbomSha256, "Native SBOM is not manifest-bound");
invariant(manifest.provenance?.file === "runa-native-bridge.provenance.json" && manifest.provenance.sha256 === provenanceSha256, "Native provenance is not manifest-bound");
invariant(identity.schema === "runa.native-windows-identity.v1", "Native identity receipt schema is invalid");
...
invariant(identity.version?.companyName === "Runa Laboratories", "Native CompanyName is invalid");
invariant(identity.version?.fileDescription === "Runa Native Credential and Browser Bridge", "Native FileDescription is invalid");
...
invariant(Array.isArray(sbom.packages) && sbom.packages.some((entry) =>
  entry?.name === "runa-native-bridge" && entry?.versionInfo === manifest.nativeVersion
), "Native SPDX document omits the native package");
invariant(provenance.schema === "runa.native-bridge-provenance.v1", "Native provenance schema is invalid");
...
invariant(provenance.signature?.identityReceipt?.file === "runa-native-bridge.identity.json", "Native provenance identity receipt path is invalid");
