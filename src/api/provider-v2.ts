...
function valid(v:unknown,s:Schema):boolean {
 ...
 if(s.allOf&&!s.allOf.every(p=>valid(v,p)))return false;
 ...
 if(s.type==='object'||s.properties||s.required) {if(!v||typeof v!=='object'||Array.isArray(v))return false;const row=v as Record<string,unknown>;return !s.required?.some(k=>!Object.hasOwn(row,k))&&Object.keys(row).every(k=>s.properties?.[k]?valid(row[k],s.properties[k]):s.additionalProperties!==false);}
 return true;
}
function check(v:unknown,name:string){if(!valid(v,schemas[name]!))throw contractViolation('provider_v2_exact_contract');}
export type ProviderAgent='opencode'|'codex'|'claude-code';
export type ProviderPreset=({kind:'provider_preset';agent:'opencode'}|{kind:'native_interactive';agent:'codex'|'claude-code'})&{label:string;profile_id:string;profile_revision:number};
...
export type ProviderSessionInput={agent:ProviderAgent;operation_id:string;execution_workspace_id:string;workspace_generation:number;cwd:string;profile_id:string;profile_revision:number};
export function decodeProviderSession(v:unknown,machineId:string,input:ProviderSessionInput):AgentSessionWorkspaceEnvelope {
 ...
 if(result.machineId!==machineId||result.executionWorkspaceId!==input.execution_workspace_id||result.workspaceGeneration!==input.workspace_generation||result.agentSession.cwd!==input.cwd||result.agentSession.agent!==input.agent||result.agentSession.authMode!=='interactive_login'||s.machine_id!==machineId||s.agent_session_id!==result.agentSession.id||s.operation_id!==input.operation_id||s.profile_id!==input.profile_id||s.profile_revision!==input.profile_revision)throw contractViolation('provider_v2_create_scope');return result;
}

export function providerSessionBody(input:ProviderSessionInput){const body={version:"2",...input,auth_mode:"interactive_login"};check(body,"ProviderSessionCreateRequestV2");return body;}
