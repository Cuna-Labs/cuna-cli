...
export async function withProviderLaunchIntent<T>(input:{stateDirectory:string;ownerId:string;workspaceId:string;machineId:string;executionWorkspaceId:string;intent:Readonly<Record<string,unknown>>;confirmNew?:()=>Promise<boolean>;create:(operationId:string)=>Promise<T>}):Promise<T>{
 ...
 try{
  ...
  if(pending.length===0&&resolved&&!(await input.confirmNew?.())){if(resolved.digest!==digest)throw new CunaError({code:'cuna.provider.pending_intent_conflict',message:'Resume requires the previously selected Workspace version and preset. Explicitly confirm a new session to change them.',exitCode:EXIT_CODES.conflict});return await input.create(resolved.operationId);}
