...
const COMMAND_HELP: Readonly<Record<string, string>> = Object.freeze({
  ...
  machines: topic(
    "Usage:\n  cuna machines\n  cuna machines <list|create|start|pause|resume|stop|update-supervisor|live-update-supervisor|delete> [options]",
    [
  ...
  "machines live-update-supervisor": topic(
    ...
    [
      ...
      "",
      "When the outcome is unknown, Cuna does not repeat it and never falls back to",
      "stopping the machine. It records one local note for this machine, points you",
      "at the two reads that settle the question, and refuses the next attempt until",
      "you clear the note yourself. That note covers this machine on this API for",
      "every profile on this computer, so --profile does not clear it.",
      "",
      "Required:",
      "  --yes               Confirm this mutating operation",
      "",
      "Options:",
      "  --forget-unknown    Drop this computer's local note about an update whose",
      "                      outcome it never saw. Sends nothing, changes nothing on",
      "                      the server, and proves nothing about what happened.",
      "                      Cannot be combined with --yes.",
      "",
      "This operation can take minutes. --timeout-ms lowers its budget below what",
      "the server needs and turns a slow answer into an unknown outcome.",
    ].join("\n"),
  ),
