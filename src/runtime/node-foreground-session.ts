...
async function runNodeForegroundSessionsOnce(
  ...
  let frame = 0;
  const paint = (step: string): void => {
    frame += 1;
    void context.host.write(switchingScreen(switching.title, step, frame, context.host.dimensions(), input.color ?? true))
      .catch(() => undefined);
  };
  paint("Checking the session");
  try {
    return await runNodeForegroundSessionsAdmitted({
      ...
      onProgress: paint,
      onBeforeTerminalOwnership: () => { removeInput(); },
...
async function listMachineAgentSessions(
  ...
  } while (cursor !== undefined && items.length < 1_000);
...
function switchingScreen(
  ...
  return new TextEncoder().encode([
    ...
    dimensions.rows > 1 ? pad(` ${spinner} ${step}  ·  ${SWITCH_INPUT_NOTICE}  ·  Ctrl-C closes`) : "",
    color ? "\u001b[0m" : "",
  ].join(""));
}
