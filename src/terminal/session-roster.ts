 ...
 * own label, so it is dropped. Equal labels are told apart by the working
 * folder when it is a readable name, else by the local start time (the
 * published folder is a Workspace UUID), else by the id's first characters.
 */
export function sessionRosterEntries(records: readonly RosterRecord[]): readonly SessionRosterEntry[] {
  const bases = records.map((record) => record.name === record.agent ? "" : record.name);
  const refined = disambiguate(bases, records.map((record) =>
    record.folder !== "" && !UUID_LIKE.test(record.folder) ? record.folder : localStartTime(record.createdAt)));
  const labels = disambiguate(refined, records.map((record) => record.agentSessionId.slice(0, 4)));
...
/** Append `extra[i]` to every label that is not unique; unique labels stay as they are. */
function disambiguate(labels: readonly string[], extra: readonly string[]): readonly string[] {
  const counts = new Map<string, number>();
  for (const label of labels) counts.set(label, (counts.get(label) ?? 0) + 1);
  return labels.map((label, index) => (counts.get(label) ?? 0) > 1
    ? [label, extra[index] ?? ""].filter((part) => part.length > 0).join(" ")
    : label);
}

/** `HH:MM` on this computer's clock; empty when the timestamp does not parse. */
function localStartTime(createdAt: string): string {
...
function truncateLabel(value: string): string {
  ...
  return characters.length <= LABEL_MAXIMUM ? value : `${characters.slice(0, LABEL_MAXIMUM - 1).join("")}…`;
}
